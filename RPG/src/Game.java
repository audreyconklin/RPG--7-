...
public class Game extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	public void drawGameScreen(Graphics g2d) {
		...
		// Check if enemies queue is empty before trying to access the current enemy
		if (!enemies.isEmpty()) {
	...
	private void handleCollisionsAndCleanup() {
		...
		for (Ranged projectile : rangedWeap) {
			...
			if (!enemies.isEmpty()) {
				...
				if (checkCollision(projectile, currentEnemy)) {
		...
		// Trigger repaint if an enemy was defeated
		if (enemyDefeated) {
			repaint();
		}
	}
		...
		if (screen == "gameplay") {
			if (!enemies.isEmpty()) {
				...
				if (currentEnemy != null && random.nextBoolean()) { // 50% chance to fire
					currentEnemy.fireBack();
					System.out.println("Enemy fired back!");
